...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    time_stat["setup_time"] = time.time() - start_time
    ...
    for epoch in range(epochs_trained, args.num_train_epochs):
        ...
        # for step, batch in enumerate(epoch_iterator):
            for step, batch in enumerate(train_dataloader):
                ...
                glo.set_value('iter_this_epoch', step)
                data_index = step * args.train_batch_size
                ...
                # tr_loss += loss.item()
                tr_loss += loss.data.detach().clone()
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        dtype = next(model_to_save.parameters()).dtype
                    ...
                    print('step {}, batch time: {}'.format(step, time.time() - iter_time))
                    iter_time = time.time()
                # p.step()

        dur_ep = time.time() - start_time
        time_stat["epoch" + str(epoch)] = dur_ep
        logger.info("Epoch {} takes {}s".format(epoch, dur_ep))
    ...
    if bps.rank() == 0:
        for k, dur in time_stat.items():
            logger.info("Time stat {} : {}s".format(k, dur))
